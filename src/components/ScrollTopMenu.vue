  ...
  <div class="container box">
    ...
    <div class="list-con box box-y-center mescroll-touch-x box-col-flex">
      <ul class="list" :style="{ margin: lessCenter ? '0 auto' : '' }">
        <li @click="changeItem(item.key ? item.key : key)" :class="{active: activeKey === (item.key ? item.key : key)}" v-for="(item, key) in list" :key="key">{{ $Helper.isType(item, '') ? item : item.text }}</li>
      </ul>
    </div>
